...
def compute_stationary_cov(A, Q, max_iter=2000):
    """
    Solve discrete Lyapunov equation: Sigma = A Sigma A^T + Q
    using jax.lax.fori_loop for JIT compatibility.
    Runs for exactly max_iter iterations (no early stopping in JIT).
    ...
    Sigma = jax.lax.fori_loop(0, max_iter, step, Q)
    return Sigma
